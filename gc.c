...
void start_sample_thread(struct f2fs_sb_info *sbi){
	//开启线程之前做一些初始化工作
	sbi->sample_irr_array = vzalloc(sizeof(unsigned int) * 90000);
	sbi->sample_lws_array = vzalloc(sizeof(unsigned int) * 90000);
	sbi->last_utilization = 1000;
